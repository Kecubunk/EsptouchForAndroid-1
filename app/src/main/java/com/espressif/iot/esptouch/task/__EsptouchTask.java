...
import com.espressif.iot.esptouch.demo_activity.MLog;
...
public class __EsptouchTask implements __IEsptouchTask {
	...
	private Handler __EsptouchTaskHandler;
	private MLog log;

	public __EsptouchTask(String apSsid, String apBssid, String apPassword,
			Context context, IEsptouchTaskParameter parameter,
			boolean isSsidHidden, Handler handler) {
		__EsptouchTaskHandler = handler;
		log = new MLog(__EsptouchTaskHandler);
		...
		mSocketClient = new UDPSocketClient(__EsptouchTaskHandler);
		mParameter = parameter;
		mSocketServer = new UDPSocketServer(mParameter.getPortListening(),
				mParameter.getWaitUdpTotalMillisecond(), context, __EsptouchTaskHandler);
